...
bool save_to_file(int *_src_ids, int *_dst_ids, float *_weights, int _vertices_count, long long _edges_count, string _file_name, bool _weighted, int *neigbors)
{
    ...
    graph_file << _vertices_count;
    graph_file << ' ';
    graph_file << _edges_count;
    graph_file << '\n';

    for (long long i = 0; i < _edges_count; i++)
    {

        graph_file << _src_ids[i];
        graph_file << ' ';
        graph_file << _dst_ids[i];
        graph_file << ' ';
        graph_file << neigbors[_src_ids[i]];


        // graph_file.write(_dst_ids[i], sizeof(int));
        // graph_file.write(' ', sizeof(char));
        // graph_file.write(_src_ids[i], sizeof(int));
        // graph_file.write('\n', sizeof(char));
...
bool exists(int* col_ind, int first_index,  int last_index, int val){
...
int main(int argc, char **argv)
{
    try
    {
        ...
        int* neigbors = new int[vertices_count];
        // int* row_ptr = new int[vertices_count + 1];
        // int* col_ind = new int[edges_count];
        // int counter = 0;
        // for(int i=0;i < vertices_count;i++){
        //   row_ptr[i] = counter;
        //   for(int j=0;j < edges_count;j++){
        //     if(src_ids[j] != i) continue;
        //     if(dst_ids[j] == i) continue;
        //     if(!exists(col_ind, row_ptr[i], counter, dst_ids[j])){
        //       col_ind[counter] = dst_ids[j];
        //       neigbors[i]++;
        //       counter++;
        //     }
        //   }
        ...
        t1 = omp_get_wtime();
        save_to_file(src_ids, dst_ids, weights, vertices_count, edges_count, file_name, weighted, neigbors);
        t2 = omp_get_wtime();
